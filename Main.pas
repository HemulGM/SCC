...
uses
  ...
  Dialogs, ExtCtrls, StdCtrls, XPMan, Menus, CheckLst, Buttons,
  ExtDlgs, PanelExt, pngimage, ActnList, ComCtrls, System.Actions,
...
type
  TFormMain = class(TForm)
    ...
    procedure DrawPanelCanResize(Sender: TObject; var NewWidth,
    ...
    procedure PanelPointsMouseDown(Sender: TObject; Button: TMouseButton;
      Shift: TShiftState; X, Y: Integer);
    procedure PanelPointsMouseMove(Sender: TObject; Shift: TShiftState; X,
      Y: Integer);
    ...
    procedure CheckListBoxPointsMouseDown(Sender: TObject;
...
implementation
 uses Math, Functions, MMSystem, CGroup, CFunction, WaitDialog, About, GoToXY;
 type
...
procedure ShowInfo(IText:string; TOut:Integer); overload;
begin
 if InfoText = '' then InfoText:=IText else InfoText:=InfoText+', '+IText;
...
begin
 case Quarter of
  tqNo:Result:='Четверть: Центр';
  tqI:Result:='Четверть: I';
  tqII:Result:='Четверть: II';
  tqIII:Result:='Четверть: III';
  tqIV:Result:='Четверть: IV';
  tqI_II:Result:='Четверть: I - II';
  tqII_III:Result:='Четверть: II - III';
  tqIII_IV:Result:='Четверть: III - IV';
  tqIV_I:Result:='Четверть: IV - I';
 end;
end;
...
begin
 ...
 if (OldMousePos.X-CenterPos.X = 0) and (OldMousePos.Y-CenterPos.Y = 0) then Result:=tqNo   else
 if (OldMousePos.X-CenterPos.X = 0) and (OldMousePos.Y-CenterPos.Y < 0) then Result:=tqI_II   else
 if (OldMousePos.X-CenterPos.X < 0) and (OldMousePos.Y-CenterPos.Y = 0) then Result:=tqII_III else
 if (OldMousePos.X-CenterPos.X = 0) and (OldMousePos.Y-CenterPos.Y > 0) then Result:=tqIII_IV else
 if (OldMousePos.X-CenterPos.X > 0) and (OldMousePos.Y-CenterPos.Y = 0) then Result:=tqIV_I   else
 //Основные, привилегирующие "Четверти"
 if (OldMousePos.X-CenterPos.X > 0) and (OldMousePos.Y-CenterPos.Y < 0) then Result:=tqI      else
 if (OldMousePos.X-CenterPos.X < 0) and (OldMousePos.Y-CenterPos.Y < 0) then Result:=tqII     else
 if (OldMousePos.X-CenterPos.X < 0) and (OldMousePos.Y-CenterPos.Y > 0) then Result:=tqIII    else
 if (OldMousePos.X-CenterPos.X > 0) and (OldMousePos.Y-CenterPos.Y > 0) then Result:=tqIV;
end;
  ...
  begin
   ...
   for i:=1 to 100 do
    begin
     ...
     else CheckListBoxPoints.Checked[CheckListBoxPoints.Items.Add(Format('Группа № %d', [i]))]:=EnginePoints.Groups[i].Used;
    end;
...
var X, Y, XXMax, XXMin:Extended;
 i, PUp, PupInc:Cardinal;
 DoRetry, IsError:Boolean;
 label Retry;
begin
 if YourFunction.Computed then Exit;                                            //Если уже посчитан - выходим
 ...
 if YourFunction.StepX = 0 then Exit;
 ...
 while X <= XXMax do
  begin
   ...
   Retry:Application.ProcessMessages;                                           //Прыжек сюда, если пользователь хочет повторить (все равно не поможет =))
   ...
   try
    ...
    begin
     case MessageBox(Application.Handle, PChar(Format('Произошла ошибка при вычислении фцнкции графика.'#13#10' При X = %g', [X])), 'Внимание', MB_ICONEXCLAMATION or MB_ABORTRETRYIGNORE) of
      ...
      idRetry: DoRetry:=True;                                                   //Повторить
      idIgnore: IsError:=True;                                                  //Пропустить точку
     end;
    end;
   end;
   if DoRetry then goto Retry;                                                  //Если повтор - несколько шагов назад
...
begin
 Result:=False;
 if FID > Length(FunctionsEngines) then Exit;
...
begin
 if Length(FunctionsEngines) > 0 then
  for i:= 0 to Length(FunctionsEngines)-1 do
   begin
    if FunctionsEngines[i].Use then Continue;
...
begin
 ...
 if not (Result in [1..100]) then Result:=1;
...
begin
 ...
 AGID:=Engine.GID;
 R:=10 / Engine.Step;
 if Length(Groups[AGID].Points) <= 0 then Exit;
 ...
 if Length(Groups[AGID].Points) = 1 then
  begin
   //Result:=Diff < R;
   if Diff < R then Result:=PID;
   Exit;
  end;
 for i:=Length(Groups[AGID].Points)-1 downto 1 do
  begin
   if not Groups[AGID].Points[i].Used then Continue;
 ...
 if Diff < R then Result:=PID else Result:=-1;
 //Result:=Diff < R;
end;
...
begin
 if Length(Groups[GID].Points) > 0 then
  for i:=0 to Length(Groups[GID].Points) do
   begin
    if Groups[GID].Points[i].Used then Continue;
...
var i:Cardinal;
    ...
    D2:TDirect2DCanvas;
begin
 if not YourFunction.Computed then Exit;
 D2:=TDr
 with Draw.Canvas do
  begin
   ...
   then RPoint:=Point(10, Height div 2);
...
begin
 with Engine, Engine.Parameters, Engine.Draw.Canvas do
  begin
   ...
   Pen.Width:=LineSize;
   if Step >=20 then TextOut(CenterPos.X-9, CenterPos.Y+3, '0');    //Если масштаб позволяет, рисуем 0 в начале координат
   Count:=0;
   ...
   repeat
    ...
    if Step >=20 then TextOut(i-(TextWidth('-'+IntToStr(Count)) div 2), CenterPos.Y+StepSize+5, '-'+IntToStr(Count));
    Dec(i, Step);
   until i < 0-Step;
   ...
   repeat
    ...
    if Step >=20 then TextOut(CenterPos.X+StepSize+5, i-(TextWidth(IntToStr(Count)) div 2), IntToStr(Count));
    Dec(i, Step);
   until i <= 0-Step;
...
begin
 with Engine, Engine.Parameters, EnginePoints, Engine.Draw.Canvas do
  begin
   for di:=1 to 100 do
    begin
     if not Groups[di].Used then Continue;
     if not Groups[di].ConnectPoints then Continue;
     Count:=0;
     //SetLength(UPoints, 0);
     for i:=0 to Length(Groups[di].Points)-1 do
      begin
       if not Groups[di].Points[i].Used then Continue;
...
begin
 with Engine.Parameters do                                                      //Рисуем, что приспичит
  begin
   ...
   if DrawGrid then DrawingGrid;                                                //Если нужно, рисуем сетку
   DrawingAxis;                                                                 //Рисуем оси
   if ZeroPoint then DrawingZeroPoint;                                          //Если нужно, рисуем точку в центре
   DrawingInfo;                                                                 //Выводим осн. инф.
   if ShowAdvInfo then DrawingAdvInfo;                                          //Если нужно, отображаем доп. инф.
   DrawingFissinos;                                                             //Рисуем отрезки, их деления и если масштаб позволяет - подписи
   //DrawingAutor;                                                                //Рисуем меня
   if DrawFunction then DrawingFunction;                                        //Если нужно, высчитываем и рисуем график указанной функции
   if ConnectPoints then DrawingConnects;                                       //Если нужно, соединяем пользовательские точки, групп которые нужны
   if ShowUserPoints then DrawingPoints;                                        //Если нужно, рисуем пользовательские точки групп, которые нужны
   if ShowCursor then DrawingCursor;                                            //Если нужно, рисуем курсор
   if not DrawPanel.Enabled then DrawBlackly;                                   //Если холст не активен, то затемняем фон (в разработке)
  end;
...
procedure TFormMain.DrawPanelCanResize(Sender: TObject; var NewWidth,
  NewHeight: Integer; var Resize: Boolean);
begin
 SetPosPointsPanel(NewWidth, NewHeight);
 Resize:=True;
 if Engine.Height <> NewHeight then Engine.Height:=NewHeight;
 if Engine.Width <> NewWidth then Engine.Width:=NewWidth;
 ...
 case Button of
  mbLeft:
   begin
    ...
    if ssCtrl in Shift then EnginePoints.Add('Точка', ExtPoint(Round((OldMousePos.X-Engine.CenterPos.X) / Engine.Step), Round(-(OldMousePos.Y-Engine.CenterPos.Y) / Engine.Step)), Engine.GID) else
  ...
  mbRight:
   begin
    ...
    else PopupMenuContext.Popup(MPos.X, Mpos.Y);
   end;
  mbMiddle:
   begin
...
procedure TFormMain.DrawPanelMouseMove(Sender: TObject; Shift: TShiftState;
  X, Y: Integer);
begin
 if (not DrawPanel.Focused) and (not EditPointName.Visible) then DrawPanel.SetFocus;
 if Engine.Parameters.ShowCursor then Engine.CursorPoint.Coord:=ExtPoint(((X-Engine.CenterPos.X) / Engine.Step), ((Y-Engine.CenterPos.Y) / Engine.Step));
 if MouseIsDown then Engine.CenterPos:=Point(Engine.CenterPos.X+(X-OldMousePos.X), Engine.CenterPos.Y+(Y-OldMousePos.Y));
 OldMousePos:=Point(X, Y);
 ...
 if not TimerStartMove.Enabled then TimerAutoMove.Enabled:=False;
end;
...
begin
 if DXTimer.Interval <=1 then TimerUpSpeed.Enabled:=False;
...
begin
 if MouseIsDown then Screen.Cursor:=2 else Screen.Cursor:=crDefault;
end;
...
begin
 if DrawFPS then
  begin
   Inc(CurAdd);
   if FPS > 0 then EnginePoints.Add('', ExtPoint(CurAdd/100, FPS/1000), 5)
   else EnginePoints.Add('', ExtPoint(CurAdd/100, 0), 5);
  end;
...
begin
 ...
 if not EnginePoints.Delete(ExtPoint(A1, -A2)) then MessageBox(Handle, 'Точка с указанными данными не существует!', '', MB_ICONWARNING or MB_OK);
end;

procedure TFormMain.TimerAutoMoveTimer(Sender: TObject);
begin
 if OldMousePos.X<=Engine.BordersOfTheMoving then Inc(Engine.CenterPos.X, Engine.Step div 2);
 if OldMousePos.Y<=Engine.BordersOfTheMoving then Inc(Engine.CenterPos.Y, Engine.Step div 2);
 if OldMousePos.X>=DrawPanel.Width-Engine.BordersOfTheMoving then Dec(Engine.CenterPos.X, Engine.Step div 2);
 if OldMousePos.Y>=DrawPanel.Height-Engine.BordersOfTheMoving then Dec(Engine.CenterPos.Y, Engine.Step div 2);
end;
...
var UserPoint:TUserPoint;
begin
 if IsRename then Exit;
 if not EnginePoints.UnderCursor(RenamedPoint, RenamedGroup, UserPoint) then Exit;
...
procedure TFormMain.DrawPanelMouseWheelDown(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
var VOld, NOld:TExtPoint;
begin
 if Engine.Step <= 1 then Exit;
...
procedure TFormMain.DrawPanelMouseWheelUp(Sender: TObject;
  Shift: TShiftState; MousePos: TPoint; var Handled: Boolean);
var VOld, NOld:TExtPoint;  
begin
 if Engine.Step >= 1800 then Exit;
 Engine.Step:=Engine.Step+1;
 SpinEditorStep.Value:=Engine.Step;
 VOld:=ExtPoint(Engine.CursorPoint.Coord.X, -Engine.CursorPoint.Coord.Y);
 DrawPanelMouseMove(Sender, Shift, OldMousePos.X, OldMousePos.Y);
...
begin
 case Key of
  83: Dec(Engine.CenterPos.Y, Engine.Step);
  87: Inc(Engine.CenterPos.Y, Engine.Step);
  65: Inc(Engine.CenterPos.X, Engine.Step);
  68: Dec(Engine.CenterPos.X, Engine.Step);
  VK_ESCAPE:if Assigned(Menu) then Menu:=nil else Menu:=MainMenu;
  VK_RETURN: if Shift = [ssAlt] then ActionFullScreen.Execute;
  Ord('F'): if Shift = [ssAlt, ssCtrl] then
             if DrawFPS then
              begin
               DrawFPS:=False;
               CurAdd:=0;
...
var SelPoint:TGroupOfUserPoints;
begin
 Engine.GID;
...
procedure TFormMain.PanelPointsMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
...
procedure TFormMain.PanelPointsMouseMove(Sender: TObject;
  Shift: TShiftState; X, Y: Integer);
begin
 if PanelPoints.Left <=0 then PanelPoints.Left:=0;
 if PanelPoints.Top <=0 then PanelPoints.Top:=0;
 if PanelPoints.Top+PanelPoints.Height >= DrawPanel.Height then PanelPoints.Top:=DrawPanel.Height-PanelPoints.Height;
 if PanelPoints.Left+PanelPoints.Width >= DrawPanel.Width then PanelPoints.Left:=DrawPanel.Width-PanelPoints.Width;
end;
...
begin
 ...
 case (Sender as TCheckBox).Tag of
  1:EnginePoints.Groups[Engine.GID].ShowPointsLines:=(Sender as TCheckBox).Checked;
  2:EnginePoints.Groups[Engine.GID].ShowPointsName:=(Sender as TCheckBox).Checked;
  3:EnginePoints.Groups[Engine.GID].ShowPoint:=(Sender as TCheckBox).Checked;
  4:EnginePoints.Groups[Engine.GID].ConnectPoints:=(Sender as TCheckBox).Checked;
 end;
end;
...
begin
 if not DrawPanel.Enabled then PlaySound(PChar('SYSTEMQUESTION'), 0, SND_ASYNC);
end;

procedure TFormMain.CheckListBoxPointsMouseDown(Sender: TObject;
  Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var MPos:TPoint;
begin
...
begin
 if IsFullScreen then RestoreDefaultMode;
end;

procedure TFormMain.FormActivate(Sender: TObject);
begin
 if IsFullScreen then HideTask;
end;

procedure TFormMain.FormDeactivate(Sender: TObject);
begin
 if IsFullScreen then ShowTask;
end;

procedure TFormMain.ActionScreenShotExecute(Sender: TObject);
begin
 if SavePictureDialogFrame.Execute then Engine.Draw.SaveToFile(SavePictureDialogFrame.FileName);
end;
...
begin
 if IsFullScreen then RestoreDefaultMode else IsFullScreen:=SetFullscreenMode;
 ActionFullScreen.Checked:=not IsFullScreen;
 if Engine.Height <> DrawPanel.Height then Engine.Height:=DrawPanel.Height;
 if Engine.Width <> DrawPanel.Width then Engine.Width:=DrawPanel.Width;
end;
...
begin
 ...
 if MessageBox(Handle, PChar(Format('Вы действительно хотите удалить все точки группы № %d?', [gd])), '', MB_ICONINFORMATION or MB_YESNO) <> ID_YES then Exit;
 for i:=0 to Length(EnginePoints.Groups[gd].Points)-1 do EnginePoints.Groups[gd].Points[i].Used:=False;
 ...
 else ShowInfo(Format('Удалены все точки группы № %d', [gd]))
end;
...
begin
 if not (Sender is TsColorSelect) then Exit;
 case (Sender as TsColorSelect).Tag of
  1:EnginePoints.Groups[Engine.GID].PointBrush:=(Sender as TsColorSelect).ColorValue;
  2:EnginePoints.Groups[Engine.GID].PointPen:=(Sender as TsColorSelect).ColorValue;
  3:EnginePoints.Groups[Engine.GID].PointsLinesColor:=(Sender as TsColorSelect).ColorValue;
 end;
end;
